...
!(function () {
    function factory($, Common, Form, Pager, Dialog) {
        ...
        function render(option) {
            ...
            option.autoSave = option.autoSave === undefined ? true : option.autoSave;
            option.enterSave = option.enterSave === undefined ? true : option.enterSave;
            option._idCount = 0;
            ...
            renderPage(option);
            setArea(option, option.width, option.height);
            ...
            option.cancel = function (id, field) {
                cancel(option, id, field);
            }

            option.edit = function (id, field) {
                edit(option, id, field);
            }
            ...
            option.setArea = function (width, height) {
                return setArea(option, width, height);
            }
        ...
        function addRow(option, _option) {
            ...
            function _addRow(fixed) {
                ...
                var tr = null;
                if (fixed == 'left') {
                    $table = option.$fixedLeftTable;
                } else if (fixed == 'right') {
                    $table = option.$fixedRightTable;
                }
                if (_option.id) {
                ...
                if (tr) {
                    data.reverse().map(function (item, i) {
                        var $tr = createTr(option, item, null, fixed);
                        $tr.insertAfter(tr);
                    });
                } else {
        ...
        function deleteRow(option, id) {
            ...
            function _deleteRow(fixed) {
                var $tr = null;
                if (fixed == 'left') {
                    $tr = option.$fixedLeftTable.find('tr[data-id="' + id + '"]');
                } else if (fixed == 'right') {
                    $tr = option.$fixedRightTable.find('tr[data-id="' + id + '"]');
                } else {
                    $tr = option.$table.find('tr[data-id="' + id + '"]')
                }
                // 删除溢出内容弹框
        ...
        function save(option, id, field) {
            ...
            if (id !== undefined) { // 保存某一行的数据
                if (field) {
                    ...
                    option.$table.find('tr[data-id="' + id + '"]').each(_filter);
                ...
                option.$table.find('td').each(_filter);
                if (option.$fixedLeftTable) {
         ...
         * @param {String} field 
         */
        function cancel(option, id, field) {
            var tds = [];
            if (id !== undefined) { // 保存某一行的数据
                if (field) {
                    var td = getTdById(option, id, field);
        ...
        function edit(option, id, field) {
            ...
            if (id !== undefined) { // 编辑某一行的数据
                if (field) {
                    ...
                    option.$table.find('tr[data-id="' + id + '"]').children('td').each(_filter);
                    if (option.$fixedLeftTable) {
        ...
        function setData(option, _option) {
            ...
            var $tr = option.$table.find('tr[data-id="' + id + '"]');
            var editFields = [];
         ...
         * @param {Number} height 
         */
        function setArea(option, width, height) {
            option.width = Number(width || option.width);
            option.height = Number(height || option.height);
            if (option.width) {
                option.$view.css({
                    width: width + 'px'
                });
            ...
            option.$fixedRight && setCellWidth(option, 'right');
        }
         ...
         * @param {String} fixed 
         */
        function setCellWidth(option, fixed) {
            var ws = [];
            ...
            if (fixed == 'left') { // 左侧固定表格
                option.$fixedLeftTableHeaderHead.find('tr').each(_eachLeft);
                option.$fixedLeftTable.find('tr').each(_eachLeft);
            } else if (fixed == 'right') { // 右侧固定表格
                option.$fixedRightTableHeaderHead.find('tr').each(_eachRight);
                option.$fixedRightTable.find('tr').each(_eachRight);
            } else { // 主表格
                option.$tableHeaderHead.find('tr').each(_eachLeft);
                option.$table.find('tr').each(_eachLeft);
            }
        ...
        function renderTableHeader(option, fixed) {
            ...
            function _renderSortIcon($th, col) {
                ...
                $up.on('mouseenter', function () {
                    ...
                    option.$view.find('div.' + tableClass.sortConfirm).removeClass(tableClass.sortConfirm);
                    sortObj.field = col.field;
                    sortObj.sort = 'asc';
                    $up.addClass(tableClass.sortConfirm);
                    renderTableBody(option, true);
                    return false;
                });
                ...
                if (option.$table[0].clientHeight > option.$tableMain[0].clientHeight) {
                    if (!option.$mend) {
                        ...
                        if (option.$fixedRightHeader) {
                            ...
                            option.$tableHeader.append(option.$mend);
                        }
                    }
                } else {
                    option.$mend && option.$mend.remove();
                }
            }
        }
        ...
        function renderTableFixed(option) {
            ...
            if (option.$fixedLeft || option.$fixedRight) {
                setArea(option, option.width, option.height);
            }
        }
        ...
        function renderTr(option, fixed) {
            ...
            for (var i = 0; i < data.length; i++) {
                $table.append(createTr(option, data[i], widths, fixed));
            }
        ...
        function createTr(option, data, widths, fixed) {
            ...
            if (id === undefined) {
                if (fixed == 'left') {
                    id = option._fixeLeftIdCount++;
                } else if (fixed == 'right') {
                    id = option._fixeRightIdCount++;
                } else {
                    id = option._idCount++;
                }
            }
        ...
        function createTd(option, col, data, width) {
            ...
            if (col.type == 'text') { //文本列
                ...
                $cell = $('<div class="' + ['song-row', tableClass.cell].join(' ') + '">' + html + '</div>');
        ...
        function bindViewEvent(option) {
            ...
            $body.on('click', function (e) {
                ...
                if (option.autoSave && table != option.$table[0] &&
