!(function (window) {
    function factory($, Common) {
        ...
        Class.prototype.render = function () {
            ...
            this.originData = Common.deepAssign([], this.option.data);
            this.value = Common.deepAssign([], this.option.value);
            ...
            this.appendItem(this.originData, this.$leftBody);
            this.appendItem(this.value, this.$rightBody);
            this.setBtnActive();
            this.setAllChecked();
            this.setEmpty();
            this.$left.css({
        ...
        Class.prototype.bindEvent = function () {
            ...
            this.$transfer.delegate('.' + transferClass.title, 'click', function () {
                var $this = $(this);
                var checked = false;
                var $tansferBody = $this.next('.' + transferClass.leftBody);
                var dataList = that.getEnable(that.originData);
                if (!$tansferBody.length) {
                    $tansferBody = that.$rightBody;
                    dataList = that.getEnable(that.value);
                }
            ...
            this.$leftBtn.on('click', function () {
                var checkedData = that.getChecked(that.value);
                if (checkedData.length) {
                    that.originData = that.originData.concat(checkedData);
                    that.value = that.value.filter(function (item) {
                    ...
                    // 触发change事件
                    that.trigger('change', {
                        data: that.value.map(function (item) {
                            return Common.delInnerProperty(item);
                        }),
                        dom: this
                    });
            ...
            this.$rightBtn.on('click', function () {
                var checkedData = that.getChecked(that.originData);
                if (checkedData.length) {
                    that.value = that.value.concat(checkedData);
                    that.originData = that.originData.filter(function (item) {
                        return !item._song_checked
                    });
                    checkedData.map(function (item) {
                        item._song_checked = false;
        ...
        Class.prototype.setAllChecked = function () {
            ...
            var enableData = null;
            checkedData = this.getChecked(this.originData);
            enableData = this.getEnable(this.originData);
            ...
            checkedData = this.getChecked(this.value);
            enableData = this.getEnable(this.value);
            if (checkedData.length == enableData.length && enableData.length) {
